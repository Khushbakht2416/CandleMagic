                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <p>Add and manage candles effortlessly!</p>
                            <div class="text-right mb-3">
                                <a href="{{ url('/admin/add-shop') }}" class="btn btn-primary"
                                    style="color: white;">
                                    <i></i> Add New Product
                                </a>
                            </div>
                            <label for="brandFilter" style="color: #354cc1">Filter by Brand:</label>
                            <select id="brandFilter" class="form-control mb-3">
                                <option value="">All Brands</option>
                                @foreach ($shop->unique('company') as $row)
                            ...
                            <table class="table" id="shopTable">
                                ...
                                <tbody>
                                    @foreach ($shop as $row)
                                        <tr class="shop-row" data-brand="{{ $row->company }}">
                                            <td>{{ $row->id }}</td>
                                            <td>
                                            ...
                                            <td>
                                                @if ($row->status == 0)
                                                    <a href="{{ url('/admin/enable-shop/' . $row->id) }}" class="btn btn-warning">
                                                        <i class="fas fa-toggle-off"></i>
                                                    </a>
                                                @else
                                                    <a href="{{ url('/admin/disable-shop/' . $row->id) }}" class="btn btn-success">
                                                        <i class="fas fa-toggle-on"></i>
                                                    </a>
                                                @endif
                                            </td>
                                            <td>
                                                <a href="{{ url('/admin/edit-shop/' . $row->id) }}" class="btn btn-primary">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <a href="{{ url('/admin/delete-shop/' . $row->id) }}" class="btn btn-danger">
    ...
    <script>
        document.getElementById('brandFilter').addEventListener('change', function () {
            ...
            document.querySelectorAll('.shop-row').forEach(row => {
                let brand = row.getAttribute('data-brand').toLowerCase();
